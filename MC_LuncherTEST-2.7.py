...
- 原版下载
- Forge 下载 + 静默安装（暂时禁用按钮，避免和 Fabric 冲突）
...
def make_log_fn(log_callback):
    """统一处理 log_callback：None → print / pyqtSignal → .emit / 普通方法 → 直接用"""
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("DHML 启动测试 v2.7")
