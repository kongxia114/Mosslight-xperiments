...
v2.8 变化：
- 下拉框加「本地版本」分组，Forge/Fabric 等本地版本能显示
- Fabric 安装完成自动选中新版本
- 官方版本条目：userData 是 {"id", "url", "sha1", ...}
- 本地版本条目：userData 是 {"id", "_local": True}
- on_download 拒绝本地版本
- on_install_fabric 拒绝给本地版本装 Fabric
"""
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("DHML 启动测试 v2.8")
    ...
    def _on_manifest_loaded(self, versions):
        ...
        local_installed = self._get_installed_versions()      # set[str]
        official_ids = {v["id"] for v in versions}

        # ---- 1. 本地版本（不在 BMCLAPI 清单里的） ----
